...
return ((vm, host) => {
	...
	global.setTimeout = function(callback, delay, ...args) {
		if (!TIMERS_PAUSED)
		{
			const tmr = host.setTimeout(function() {
				...
				oneTimerGC(TIMERS_SAVED.findIndex((el)=>{return el.host === tmr;}));
			}, delay);
			...
			TIMERS_SAVED.push({
				local: local,
				host: tmr,
				timeout: new Date().getTime()+delay,
				callback: callback,
				args: args
			});
	...
	global.clearTimeout = function(local) {
		if (!TIMERS_PAUSED)
		{
			let tmp = TIMERS.get(local);
			host.clearTimeout(tmp);
			TIMERS.delete(local);

			oneTimerGC(TIMERS_SAVED.findIndex((el)=>{return el.local === local;}));
			return null;
		}else{
			oneTimerGC(TIMERS_SAVED.findIndex((el)=>{return el.local === local;}));
		}
	...
	function oneTimerGC(i) {
		TIMERS_SAVED.splice(i,1);
	}

	vm.on('pause-timers', function () {
		TIMERS_PAUSED = true;
		let tm = new Date().getTime();
		for(let i = 0,l = TIMERS_SAVED.length;i < l;i++)
		{
			let saved = TIMERS_SAVED[i];
			TIMERS.delete(saved.local);
			if (typeof saved.timeout !== 'undefined') // setTimeout
			{
				host.clearTimeout(saved.host);
				saved.pausedTime = saved.timeout - tm;
			}else{ // setInterval
				host.clearInterval(saved.host);
				saved.pausedTime = (tm - saved.startTime) % saved.interval;
			}
		}
	});

	vm.on('resume-timers', function () {
		TIMERS_PAUSED = false;
		let tm = new Date().getTime();
		for(let i = 0,l = TIMERS_SAVED.length;i < l;i++)
		{
			...
			if (typeof saved.timeout !== 'undefined') // setTimeout
			{
				saved.timeout = saved.pausedTime + tm;
				if (saved.pausedTime >= 0)
				{
					const tmr = host.setTimeout(function() {
						saved.callback.apply(null, saved.args);
						oneTimerGC(TIMERS_SAVED.findIndex((el)=>{return el.host === tmr;}));
					}, saved.pausedTime);
					saved.host = tmr;
					saved.local.ref = function() { return tmr.ref(); };
					saved.local.unref = function() { return tmr.unref(); };
					TIMERS.set(saved.local,saved.host);
				}
			}else{ // setInterval
				setTimeout(function () {
					saved.callback.apply(null, saved.args);
